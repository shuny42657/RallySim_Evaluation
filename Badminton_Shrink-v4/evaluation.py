    ...
    while True:
        ...
        for t_test in range(int(args["max_episode_len"])):
            action_test = agent.select_action(
                np.reshape(state_test, (1, state_dim)), z, stochastic=False
            )
...
def main(args):
    ...
    return_test_epi = evaluate_greedy_ppo(test_env, agent, args, 0, 0, state_dim)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--result-file", help="file name for result file", default="/trials_lppo_"
    )
    parser.add_argument(
