...
def evaluate_greedy_high_level(
    env_test, high_level_agent, low_level_agent, args, test_iter, test_n, state_dim
):
    ...
    while True:
        for t_test in range(int(args["max_episode_len"])):
            ...
            if t_test % 10 == 0:
                ...
                high_level_actions = {
                    agent: high_level_agent.select_action(np.array(obs_dict[agent]))
                    for agent in env_test.agents
                }
            low_level_actions = {
                agent: low_level_agent.select_action(
                    np.reshape(obs_dict[agent], (1, state_dim)),
                    high_level_actions[agent],
                )
                for agent in env_test.agents
            }
            for agent in env_test.agents:
        ...
        for agent in env_test.agents:
            print(
                "test_iter:{:d}, nn:{:d}, return_epi_test: {:d}".format(
                    int(test_iter), int(test_n), int(returns_epi_test[agent])
                )
            )


def evaluate_greedy_low_level(
    env_test,
    low_level_agent,
    args,
    test_iter,
    ...
    while True:
        ...
        for t_test in range(int(args["max_episode_len"])):
            ...
            selected_actions = {
                agent: low_level_agent.select_action(
                    np.reshape(states_test[agent], (1, state_dim)), z, stochastic=False
                )
                for agent in env_test.agents
            }
            ...
            for agent in env_test.agents:
                lower_bound = -1.0
                upper_bound = 1.0
                selected_actions[agent] = np.clip(
                    selected_actions[agent], lower_bound, upper_bound
                )
            ##print('selected action',selected_actions[agent_to_evaluate])
            ...
            new_states, rewards_test, terminals_test, infos_test = env_test.step(
                selected_actions
            )
...
def evaluate_greedy_ppo(env_test, low_level_agent, args, test_iter, test_n, state_dim):
    ...
    while True:
        ...
        for t_test in range(int(args["max_episode_len"])):
            selected_actions = {
                agent: low_level_agent.select_action(
                    np.reshape(states_test[agent], (1, state_dim)), stochastic=False
                )
                for agent in env_test.agents
            }
            for agent in env_test.agents:
...
def evaluate_manual_input(
    ...
    low_level_actions = {}
    random_actions = {
        agent: env_test.action_space(agent).sample() for agent in env_test.agents
    }
...
def main(args):
    for ite in range(int(args["trial_num"])):
        ...
        unity_low_level_env = UnityEnvironment(
            args["high_level_env"],
            no_graphics=False,
            worker_id=int(args["port_offset_low"]),
        )
        ...
        if int(args["train_high_level"]) == 0:
            print("Only Low-leve")
            low_level_agent = PPO(20, 3, ac_kwargs=dict(hidden_sizes=(128, 64)))
            low_level_agent.load_model(
                args["iter"], args["seed"], args["high_level_env"]
            )
            evaluate_greedy_ppo(low_level_env_test, low_level_agent, args, 0, 0, 20)
            ##evaluate_greedy_high_random(low_level_env_test,low_level_agent,args,0,0,17)
        else:
            print("HRL")
            evaluate_high(
                low_level_env_test, high_level_agent, low_level_agent, args, 0, 0, 17
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--high_level_env", help="choose the gym env- tested on {Pendulum-v0}"
    )
    ...
    parser.add_argument(
        "--latent-cont-dim", default=2, type=int
    )  # dimension of the continuous latent variable
    parser.add_argument(
        "--latent-disc-dim", default=0, type=int
    )  # dimension of the discrete latent variable
    ...
    parser.add_argument(
        "--model-save-freq", help="frequency of evaluating the policy", default=200000
    )
